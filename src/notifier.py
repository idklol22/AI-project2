...
Features:
  ...
  - Configurable from config.yaml or via kwargs
"""
...
def send_alert(
    ...
    """
    Send a fault-alert email. Returns (success: bool, message: str).
    ...
    missing = [k for k in required_keys if not email_cfg.get(k)]
    if missing:
    ...
    subject = (
        f"{email_cfg.get('subject_prefix', '[FAULT ALERT]')} "
        f"{fault_class.replace('_', ' ').title()} -- {machine_id}"
    )
    body = build_html_email(fault_class, confidence, severity, machine_id)
    ...
    for attempt in range(1, max_retries + 1):
        try:
            if email_cfg.get("use_tls", True):
                ...
                with smtplib.SMTP(email_cfg["smtp_server"], email_cfg["smtp_port"], timeout=15) as server:
                    server.ehlo()
            ...
            return True, f"Alert sent successfully to {', '.join(recipients)}"
            ...
            last_error = f"Recipient(s) refused: {e}"
            break
            ...
            last_error = f"Unexpected error (attempt {attempt}): {type(e).__name__}: {e}"

        if attempt < max_retries:
            wait = 2 ** attempt
            time.sleep(wait)

    return False, f"Email failed after {max_retries} attempts. Last error: {last_error}"
...
def preview_email(
    ...
    """Return the HTML email body for preview without sending."""
    return build_html_email(fault_class, confidence, severity, machine_id)
